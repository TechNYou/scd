...
abstract contract StateMachine is ERC165, AccessControl {
  ...
  function getHistory(
    uint256 index
  ) public view returns (bytes32 fromState, bytes32 toState, address actor, uint256 timestamp) {
  ...
  function addNextStateForState(
    bytes32 state,
    bytes32 nextState
  ) public doesStateExist(state) doesStateExist(nextState) hasAdminRole(msg.sender) {
    states[state].nextStates.push(nextState);
    nextStateToState[state][nextState] = true;
  }

  function addCallbackForState(
    bytes32 state,
  ...
  function addPreConditionForState(
    bytes32 state,
